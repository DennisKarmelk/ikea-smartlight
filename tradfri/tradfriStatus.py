...
def tradfri_get_devices(hubip, userid, securityid):
    """ function for getting all tradfri device ids """
    tradfriHub = 'coaps://{}:5684/15001' .format(hubip)
    api = '{} -m get -u "{}" -k "{}" "{}" | awk \'NR==4\'' .format(coap, userid, securityid,
                                                                                tradfriHub)
...
def tradfri_get_lightbulb(hubip, userid, securityid, deviceid):
...
def tradfri_get_groups(hubip, userid, securityid):
...
def tradfri_get_group(hubip, userid, securityid, groupid):
